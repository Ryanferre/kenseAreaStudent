...
function AudioPreVialusation({ sound, from, duration }: AudioMessageProps) {
  ...
  return (
    <div className={`w-full h-full flex ${from}`}>
      <div className="relative flex items-center gap-3 p-3 rounded-xl w-full bg-black/30 backdrop-blur-md">
        <button onClick={togglePlay} className="w-8 h-8 flex items-center justify-center rounded-full transition">
          {isPlaying ? "⏸" : "▶"}
        </button>
        <div style={{ width: `${soundDuration}px` }} className="flex-1 h-2 w-full bg-white/30 rounded">
...
export default AudioPreVialusation